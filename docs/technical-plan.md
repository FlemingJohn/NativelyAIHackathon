...
> **Superseded in part.** This was written against a Next.js frontend + FastAPI
> backend split, which no longer exists. The app now lives entirely in
> [`nativebuilder/`](../nativebuilder/) — Vite + React talking to Supabase Edge
> Functions, because native.builder runs no standalone server. The product
> thinking, module contracts, and gather→extract→synthesize pipeline below all
> still hold; the stack table and repo layout are history. See
> [`nativebuilder/README.md`](../nativebuilder/README.md) for the current shape.
